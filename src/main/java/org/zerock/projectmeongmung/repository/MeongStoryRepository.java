...
public interface MeongStoryRepository extends JpaRepository<MeongStory, Long>, QuerydslPredicateExecutor<MeongStory> {
    ...
    @Query("SELECT m FROM MeongStory m WHERE m.category = 'Category3' AND (m.title LIKE CONCAT('%', :keyword, '%') OR m.content LIKE CONCAT('%', :keyword, '%'))")
    Page<MeongStory> searchNoticeByKeyword(@Param("keyword") String keyword, Pageable pageable);
